...
engine = create_engine('postgresql://user:aaa@18.230.9.46/qed')
e = engine.connect()
...
wishes_course = pd.read_sql('select canonical_course_id,university_id,cities.name as city,base_user_id from gambit.wishes inner join gambit.users on wishes.global_user_id = users.global_user_id left join cities on city_id = cities.id where canonical_course_id is not null',e)

rec_wishes_off = pd.merge(users_model, wishes_course, on='base_user_id',how='inner')
#wishes = wishes_course[['canonical_course_id','base_user_id']].drop_duplicates()
...
df = rec_wishes_off[['base_user_id','canonical_course_id']].reset_index(drop=True)
...
users_uniques = pd.concat([rec_wishes_off[['base_user_id','canonical_course_id']],users_uniques1,users_uniques2,users_uniques3])
...
modelo_v1 = pd.merge(rec_wishes_off[['base_user_id','contact_id','city', 'channel']], users_uniques[['base_user_id', 'canonical_course_id']], on=['base_user_id'],how='inner').drop_duplicates()
...
rec_modelo_v1['model_version'] = '3.0'
...
model_p = pd.merge(rec_modelo_v1, rec_wishes_off[['contact_id', 'channel','canonical_course_id']].rename(columns={"canonical_course_id": "searched"}), on=['contact_id', 'channel'],how='inner').drop_duplicates()
...
engine_2 = create_engine('postgresql://postgres:Cognitivo@qedu-recomendacao.cuwo7zs53t3u.us-east-1.rds.amazonaws.com/postgres')
e2 = engine_2.connect()
